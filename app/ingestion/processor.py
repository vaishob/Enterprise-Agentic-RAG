...
def save_processed_locally(data: dict, source_type: str, filename: str) -> str:
    """
    Saves parsed chunk metadata as JSON locally in processed_data/<source_type>/.

    Args:
        data (dict): The processed data to be saved.
        source_type (str): The type of source (e.g., 'pdf', 'html', 'text').
        filename (str): The original filename of the source.
...
def process_file(file_path: str, filename: str, source_type: str):
    ...
    with logfire.span("📂 Processing File", file=filename, source=source_type):
        try:
            ...
            logfire.info(f"✅ Saved processed data at {local_path}")
...
def process_directory(dir_path: str, source_type: str):
    ...
    with logfire.span("📁 Scanning Directory", path=dir_path, source=source_type):
        ...
        logfire.info(f"Found {len(files)} files in {dir_path} for processing.")
...
def run_universal_ingestion(base_dir: str, explicit_source_type: str = None, wipe: bool = False):
    """
    Scan the base directory for subdirectories and process files based on their source type.
    Pass --wipe to drop and recreate the Qdrant collection before ingestion.
    """
    with logfire.span("🚀 Universal Ingestion Started", base_directory=base_dir):

        # Wipe colelction if requested
        if wipe:
            with logfire.span("🧹 Wiping Qdrant Collection"):
                if qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
                    qdrant_client.delete_collection(settings.QDRANT_COLLECTION)
                    logfire.info(
                        f"Collection '{settings/QDRANT_COLLECTION}' deleted.")

        # Recreate colection - dimension is determined by the embedding model at runtime
        ...
        # Route to sub-folders or treat the whole dir as a single source type
        subdirs = [
        ...
        if not subdirs:
            if explicit_source_type:
                ...
                base_name = os.path.basename(
                    os.path.normpath(base_dir).lower())
                source_type = (
