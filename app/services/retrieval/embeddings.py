...
def _probe_gemini():
    """
    Probes the Gemini model to check if it's available and working.
    ...
    try:
        ...
        logfire.info(
            "✅ Gemini embeddings ready (gemini-embedding-2-preview, 3072-dim).")
        return model
...
def _init():
    """
    Initializes the embedding model once per process, preferring Gemini if available.
...
def get_embedding_dim() -> int:
    """
    Returns the dimensionality of the active embedding model. Call after _init().
    """
    _init()
        ...
        for attempt in range(4):
            try:
                ...
                if is_rate_limit and attempt < 3:
                    ...
                    logfire.error(
                        f"❌ Gemini embedding failed (attempt {attempt + 1}/4): {e}")
                    raise
